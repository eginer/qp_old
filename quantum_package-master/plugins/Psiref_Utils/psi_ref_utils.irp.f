...
END_PROVIDER 
 ...
 BEGIN_PROVIDER [double precision, min_delta_E_psi_non_ref, (N_states)]
&BEGIN_PROVIDER [double precision, max_delta_E_psi_non_ref, (N_states)]
 implicit none
 integer :: i,j
 double precision :: hii
 double precision :: tmp
 min_delta_E_psi_non_ref =  1d10
 max_delta_E_psi_non_ref = -1d10
 do i = 1, N_det_non_ref
   call i_h_j(psi_non_ref(1,1,i),psi_non_ref(1,1,i),N_int,hii)
   do j = 1, N_states
    tmp = psi_ref_energy_diagonalized(j) - hii
    if(tmp .le. min_delta_E_psi_non_ref(j))then
     min_delta_E_psi_non_ref(j) = tmp
    endif
    if(tmp .ge. max_delta_E_psi_non_ref(j))then
     max_delta_E_psi_non_ref(j) = tmp
    endif
   enddo
 enddo

program full_ci
  ...
  double precision :: hcc_before
  hcc_before = spin_density_at_nucleous(atom_number_hcc)
  do while (N_det < N_det_max.and.maxval(abs(pt2(1:N_st))) > pt2_max)
    ...
    print *,  '-----'
    print *,  '-----'
    print*,'Isotropic hyperfine coupling constants '
    write(*,'(I2,X,4(F16.10,X))')N_det,iso_hcc_gauss(atom_number_hcc)
    write(*,'(I2,X,4(F16.10,X))')N_det,hcc_before + H_pert_diag(1)
    E_CI_before = CI_energy
    hcc_before = iso_hcc_gauss(atom_number_hcc)

subroutine davidson_diag_mrcc(dets_in,u_in,energies,dim_in,sze,N_st,Nint,iunit,istate)
  ...
  !$OMP PARALLEL DEFAULT(NONE)                                       &
      !$OMP  SHARED(sze,H_jj,N_det_ref,dets_in,Nint,istate,delta_ii,delta_ij_ref,idx_ref)           &
!     !$OMP  SHARED(sze,H_jj,N_det_ref,dets_in,Nint,istate,delta_ii,idx_ref)           &
  ...
  !$OMP DO SCHEDULE(guided)
  do i=1,N_det_ref
    H_jj(idx_ref(i)) +=  delta_ii(i,istate) + delta_ij_ref(i,i,istate)
...
!     !$OMP SHARED(n_det_ref,n_det_non_ref,idx_ref,idx_non_ref,n,H_jj,u_0,keys_tmp,Nint,v_0,istate,delta_ij)
      !$OMP SHARED(n_det_ref,n_det_non_ref,idx_ref,idx_non_ref,n,H_jj,u_0,keys_tmp,Nint,v_0,istate,delta_ij,delta_ij_ref)
...
!     endif
  ...
  do ii=1,n_det_ref
    ...
    do jj = 1, n_det_non_ref
        j = idx_non_ref(jj)
        vt (i) = vt (i) + delta_ij_ref(ii,jj,istate)*u_0(j)
        vt (j) = vt (j) + delta_ij_ref(ii,jj,istate)*u_0(i)
    enddo
    do jj = 1, n_det_ref
        j = idx_ref(jj)
        if(j==i)cycle

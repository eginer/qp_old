...
subroutine mrcc_dress(delta_ij_, delta_ii_, delta_ij_ref_, Ndet_ref, Ndet_non_ref,i_generator,n_selected,det_buffer,Nint,iproc)
 use bitmasks
 implicit none
  ...
  double precision, intent(inout) :: delta_ij_ref_(Ndet_ref,Ndet_ref,*)
  ...
  integer                        :: idx_ref_alpha(0:Ndet_ref)
  logical                        :: good
  ...
  double precision               :: haj, phase, phase2,ha_ref_k
  ...
  double precision :: H_ref(Ndet_ref)
  ...
  do i_alpha=1,N_tq
    ...
    call filter_connected_i_H_psi0(psi_ref,tq(1,1,i_alpha),N_int,N_det_ref,idx_ref_alpha)
    H_ref = 0.d0
    do ii=1,idx_ref_alpha(0)
      k = idx_ref_alpha(ii)
      !DEC$ FORCEINLINE
      call i_H_j(psi_ref(1,1,k),tq(1,1,i_alpha),N_int,ha_ref_k)
      H_ref(k)  = ha_ref_k
    enddo
    ...
    do i_I=1,N_det_ref
       ...
       ! Part of the dressing directly inside the model space 
       ! Extra diagonal  part 
       ! delta_ij_(I,J) = d_alpha_iI * <alpha|H|J>
       integer :: index_iI
       index_iI = -1
       do ii = 1, idx_ref_alpha(0)
        k = idx_ref_alpha(ii)
        ...
        do i_state=1,N_states
         delta_ij_ref_(i_I,k,i_state) +=  H_ref(k) * dIa(i_state) 
        enddo
       enddo
       ! Diagonal  part 
       ! delta_ij_(I,I) = d_alpha_iI * <alpha|H|I>
       if(index_iI > 0)then
 !      print*,'H_ref(index_iI) = ',H_ref(index_iI)
        do i_state = 1, N_states
         delta_ij_ref_(i_I,i_I,i_state) += H_ref(index_iI) * dIa(i_state)
        enddo
       endif
...
subroutine find_triples_and_quadruples(i_generator,n_selected,det_buffer,Nint,tq,N_tq)
 ...
 implicit none
  ...
  N_tq = 0
  do i=1,N_selected
    c_ref = connected_to_ref_by_mono(det_buffer(1,1,i),psi_det_generators,Nint, &

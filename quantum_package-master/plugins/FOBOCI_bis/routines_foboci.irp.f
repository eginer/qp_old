subroutine set_intermediate_normalization(norm,i_hole)
 implicit none
 integer, intent(in) :: i_hole
 double precision, intent(out) :: norm
 integer :: i,j,degree,index_ref_generators_restart
 ...
 allocate(index_one_hole(n_det),index_one_hole_one_p(n_det),index_two_hole_one_p(N_det),index_two_hole(N_det),index_one_p(N_det))
 ...
 do i = 1, N_det
  ...
  if(n_h == 1 .and. n_p == 0)then
   if(is_the_hole_in_det(psi_det(1,1,i),1,i_hole).or.is_the_hole_in_det(psi_det(1,1,i),2,i_hole))then  
    n_one_hole +=1
    index_one_hole(n_one_hole) = i
!   print*,'psi_coef = ',psi_coef(i,1)/psi_coef(index_ref_generators_restart,1)
   else
    psi_coef(i,1) = 0.d0
   endif
  elseif(n_h == 1.and. n_p == 1)then
   n_one_hole_one_p +=1
   index_one_hole_one_p(n_one_hole_one_p) = i
  elseif(n_h == 2 .and. n_p ==1 )then
   n_two_hole_one_p +=1
   index_two_hole_one_p(n_two_hole_one_p) = i
  elseif(n_h == 2 .and. n_p ==0)then
   n_two_hole +=1
   index_two_hole(n_two_hole) = i
  elseif(n_p == 1 .and. n_h ==0)then
   n_one_p+=1
   index_one_p(n_one_p) = i
  endif
 enddo
 print*,'n_one_hole       = ',n_one_hole
 print*,'n_one_hole_one_p = ',n_one_hole_one_p
 print*,'n_two_hole_one_p = ',n_two_hole_one_p
 print*,'n_two_hole       = ',n_two_hole
 print*,'n_one_p          = ',n_one_p
 print*,'psi_coef(index_ref_generators_restart,1) = ',psi_coef(index_ref_generators_restart,1)
 if(n_one_hole.ne.0)then
   inv_coef_ref_generators_restart = 1.d0/psi_coef(index_ref_generators_restart,1)
   integer :: degree_max_hole
   ! Cancel all the the one hole one particle that are double excitations respect to the one_holes
   ! Because they will be redundant
   do i = 1, n_one_hole_one_p
    degree_max_hole = 0
    do j = 1, n_one_hole
     call get_excitation_degree(psi_det(1,1,index_one_hole(j)),psi_det(1,1,index_one_hole_one_p(i)),degree,N_int)
     if(degree > degree_max_hole)then
      degree_max_hole = degree
     endif
    enddo
    if(degree_max_hole >1)then
     psi_coef(index_one_hole_one_p(i),1) = 0.d0
    endif
   enddo
 else 
  do i = 1, n_one_hole_one_p
 ...
 do i = 1, N_det
  psi_coef(i,1) = psi_coef(i,1) * inv_coef_ref_generators_restart
 enddo
 do i = 1, N_det
...
subroutine set_intermediate_normalization_lmct(norm,i_hole)
 implicit none
 integer, intent(in) :: i_hole
 double precision, intent(out) :: norm
 integer :: i,j,degree,index_ref_generators_restart
 ...
 integer :: n_one_hole,n_one_hole_one_p,n_two_hole_one_p,n_two_hole,n_one_p
 logical :: is_the_hole_in_det
 double precision :: inv_coef_ref_generators_restart,hij,hii,accu
 ...
 do i = 1, N_det
  ...
  if(degree == 0)then
   index_ref_generators_restart = i
   inv_coef_ref_generators_restart = 1.d0/psi_coef(i,1)
   cycle
  endif
  n_h = number_of_holes(psi_det(1,1,i))
  n_p = number_of_particles(psi_det(1,1,i))
 ...
 !!!!!!!!!!!!!!!!! Treating the 1p  !!!!!!!!!!!!!!!!!
 ! you only give the contribution that comes from the good_hole
 do i = 1, n_one_p
  call i_H_j(psi_det(1,1,index_one_p(i)),psi_det(1,1,index_one_p(i)),N_int,hii)
  accu = 0.d0
  do j = 1, n_good_hole
   call i_H_j(psi_det(1,1,index_good_hole(j)),psi_det(1,1,index_one_p(i)),N_int,hij)
   accu += hij * psi_coef(index_good_hole(j),1) / (ci_electronic_energy(1) - hii)
  enddo
  psi_coef(index_one_p(i),1) = accu
 enddo

 !!!!!!!!!!!!!!!!! Treating the 1h  !!!!!!!!!!!!!!!!!
 ! you only give the contribution that comes from the good_hole
 do i = 1, n_one_hole
  call i_H_j(psi_det(1,1,index_one_hole(i)),psi_det(1,1,index_one_hole(i)),N_int,hii)
  accu = 0.d0
  do j = 1, n_good_hole
   call i_H_j(psi_det(1,1,index_good_hole(j)),psi_det(1,1,index_one_hole(i)),N_int,hij)
   accu += hij * psi_coef(index_good_hole(j),1) / (ci_electronic_energy(1) - hii)
  enddo
  psi_coef(index_one_hole(i),1) = accu
 enddo
 ! Set the wave function to the intermediate normalization
 do i = 1, N_det
  psi_coef(i,1) = psi_coef(i,1) * inv_coef_ref_generators_restart
...
subroutine set_intermediate_normalization_mlct(norm,i_particl)
 implicit none
 ...
 !!!!!!!!!!!!!!!!! Treating the 1p  !!!!!!!!!!!!!!!!!
 ! you only give the contribution that comes from the good_particles
 do i = 1, n_one_p
  call i_H_j(psi_det(1,1,index_one_p(i)),psi_det(1,1,index_one_p(i)),N_int,hii)
  accu = 0.d0
  do j = 1, n_good_particl
   call i_H_j(psi_det(1,1,index_good_particl(j)),psi_det(1,1,index_one_p(i)),N_int,hij)
   accu += hij * psi_coef(index_good_particl(j),1) / (ci_electronic_energy(1) - hii)
  enddo
  psi_coef(index_one_p(i),1) = accu
 enddo
 ...
 do i = 1, n_one_hole
  ...
  accu = 0.d0
  do j = 1, n_good_particl
   call i_H_j(psi_det(1,1,index_good_particl(j)),psi_det(1,1,index_one_hole(i)),N_int,hij)
   accu += hij * psi_coef(index_good_particl(j),1) / (ci_electronic_energy(1) - hii)
  enddo
  psi_coef(index_one_hole(i),1) = accu
 enddo
 ...
 print*,'n_one_hole       = ',n_one_hole
 print*,'n_one_hole_one_p = ',n_one_hole_one_p
 print*,'n_one_hole_two_p = ',n_one_hole_two_p
 print*,'n_two_hole_one_p = ',n_two_hole_one_p
 print*,'n_two_hole       = ',n_two_hole
 print*,'n_one_p          = ',n_one_p
 print*,'psi_coef(index_ref_generators_restart,1) = ',psi_coef(index_ref_generators_restart,1)
...
subroutine check_symetry_1h1p(i_hole,i_part,thr,test)
 ...
 subroutine provide_properties
   ...
   double precision :: accu
   if(.False.)then
    provide integrated_delta_rho_all_points_z
   endif
   if(.True.)then
    accu= 0.d0
    do i = 1, nucl_num

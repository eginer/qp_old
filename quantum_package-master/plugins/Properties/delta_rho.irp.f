...
END_PROVIDER
BEGIN_PROVIDER [double precision, integrated_delta_rho_all_points_z, (N_z_pts)]
 BEGIN_DOC
! 
! integrated_rho(alpha,z) - integrated_rho(beta,z) for all the z points 
! chosen
!
 ...
 integer :: i,j,k,l,i_z,h
 double precision :: z,function_integrated_delta_rho,c_k,c_j,n_i_h,accu
 integrated_delta_rho_all_points_z = 0.d0
 !$OMP PARALLEL DO DEFAULT(none) &
 !$OMP PRIVATE(i,h,j,k,c_j,c_k,n_i_h,i_z) &
 !$OMP SHARED(mo_tot_num,ao_num,mo_coef, &
 !$OMP   ao_integrated_delta_rho_all_points_z,one_body_spin_density_mo,integrated_delta_rho_all_points_z,N_z_pts)          
 do i_z = 1, N_z_pts
  do i = 1, mo_tot_num
    do h = 1, mo_tot_num
     n_i_h = one_body_spin_density_mo(i,h)
     ...
     do j = 1, ao_num
      ...
      do k = 1, ao_num
       c_k = mo_coef(k,h)   ! coefficient of the hth MO on the kth AO
       integrated_delta_rho_all_points_z(i_z) += c_k * c_j * n_i_h *  ao_integrated_delta_rho_all_points_z(j,k,i_z)
      enddo
     enddo
    enddo
  enddo
 ...
 accu = 0.d0
 do i = 1, N_z_pts
  accu += integrated_delta_rho_all_points_z(i)
  write(i_unit_integrated_delta_rho,*)z,integrated_delta_rho_all_points_z(i),accu
  z += delta_z
 enddo
 print*,'sum of integrated_delta_rho = ',accu
...
BEGIN_PROVIDER [ double precision, ao_integrated_delta_rho_all_points_z, (ao_num_align, ao_num, N_z_pts)]
 BEGIN_DOC
!  array of the overlap in x,y between the AO function and integrated between [z,z+dz] in the z axis
!  for all the z points that are given (N_z_pts)
 END_DOC
  ...
  integer :: power_A(3), power_B(3)
  integer :: i_z
  double precision :: z,SABpartial_direction,accu_x,accu_y,accu_z
  dim1=100
 z = z_min
 do i_z = 1, N_z_pts
 !$OMP PARALLEL DO DEFAULT(none) &
 ...
 !$OMP   ao_integrated_delta_rho_all_points_z,N_z_pts,dim1,i_z,z,delta_z)           
  do j=1,ao_num
   A_center(1) = nucl_coord( ao_nucl(j), 1 )
   A_center(2) = nucl_coord( ao_nucl(j), 2 )
   A_center(3) = nucl_coord( ao_nucl(j), 3 )
...
!      accu_z += c* overlap_z * overlap_y * SABpartial_direction(1,z,z+delta_z,A_center,B_center,power_A,power_B,alpha,beta)
       accu_z += c* overlap_x * overlap_y * SABpartial_direction(3,z,z+delta_z,A_center,B_center,power_A,power_B,alpha,beta)
      enddo
     enddo
     ao_integrated_delta_rho_all_points_z(i,j,i_z) = accu_z
   enddo
  enddo
 !$OMP END PARALLEL DO
  z += delta_z

   ...
   if (exists) then
     ...
     if (exists) then
       ...
       if(exists == .False.)then
         print*,'Watchout ! '
         print*,'determinants_mo_label is ',label
         print*,'and mo_basis_molabel  is ',mo_label
         print*,'The wave function in restart might not correspond'
         print*,'to the mos you are using ...'
       endif
     endif
   endif
   if (exists) then
...
BEGIN_PROVIDER [ integer, psi_det_size ]
 ...
 PROVIDE ezfio_filename
 logical                        :: exists
 call ezfio_has_determinants_n_det_max(exists)
 if (exists) then
   call ezfio_get_determinants_n_det_max(psi_det_size)
 else
   psi_det_size = 1
 endif
 psi_det_size = max(psi_det_size,50000)
...
&BEGIN_PROVIDER [ double precision, psi_coef_sorted_bit, (psi_det_size,N_states_diag) ]
 implicit none
 BEGIN_DOC

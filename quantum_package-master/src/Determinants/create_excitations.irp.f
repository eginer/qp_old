subroutine do_mono_excitation(key_in,i_hole,i_particle,ispin,i_ok)
 ...
 integer :: n_elec_alpha,n_elec_beta
 n_elec_alpha = 0    
 n_elec_beta = 0    

 do i = 1, N_int
  n_elec_alpha += popcnt(key_in(i,1)) 
  n_elec_beta  += popcnt(key_in(i,2))
 enddo
 if((n_elec_alpha.ne.elec_alpha_num).or.(n_elec_beta.ne.elec_beta_num))then
  i_ok = -1
 endif
end

...
double precision function general_primitive_integral(dim,            &
  ...
  n_Ix = 0
  do ix = 0, iorder_p(1)
    if (dabs(P_new(ix,1)) < 1.d-16) cycle
    a = P_new(ix,1)
    do jx = 0, iorder_q(1)
      d = a*Q_new(jx,1)
      if (dabs(d) < 1.d-16) cycle
      !DEC$ FORCEINLINE
  ...
  n_Iy = 0
  do iy = 0, iorder_p(2)
    if (dabs(P_new(iy,2)) > 1.d-16) then
      b = P_new(iy,2)
      do jy = 0, iorder_q(2)
        e = b*Q_new(jy,2)
        if (dabs(e) < 1.d-16) cycle
        !DEC$ FORCEINLINE
  ...
  n_Iz = 0
  do iz = 0, iorder_p(3)
    if (dabs(P_new(iz,3)) > 1.d-16) then
      c = P_new(iz,3)
      do jz = 0, iorder_q(3)
        f = c*Q_new(jz,3)
        if (dabs(f) < 1.d-16) cycle
        !DEC$ FORCEINLINE
